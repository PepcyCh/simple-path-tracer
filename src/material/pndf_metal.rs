...
use crate::{
    ...
    scatter::{
        FresnelConductor, MicrofacetReflect, PndfAccel, PndfGaussTerm, PndfReflect, SpecularReflect,
    },
};
...
impl LoadableSceneObject for PndfMetal {
    fn load(
        scene: &mut Scene,
        path: &std::path::PathBuf,
        json_value: &JsonObject,
    ) -> anyhow::Result<()> {
        let name = loader::get_str_field(json_value, "material-pndf_metal", "name")?;
        let env = format!("material-pndf_metal({})", name);
