...
pub struct TexInputModifier {
    ...
    tiling: glam::Vec3A,
    offset: glam::Vec3A,
    mode: TextureInputMode,
    wrap: TextureInputWrapMode,
}
...
impl TexInputModifier {
    ...
    pub fn load_with_tex(params: &mut InputParams, tex: Arc<Texture>) -> anyhow::Result<Self> {
        ...
        let (tiling, offset) = if mode == TextureInputMode::Texcoords {
...
fn load_mode(params: &mut InputParams) -> anyhow::Result<TextureInputMode> {
    if params.contains_key("mode") {
        ...
        Ok(TextureInputMode::Texcoords)
    }
}

fn load_wrap(params: &mut InputParams) -> anyhow::Result<TextureInputWrapMode> {
    if params.contains_key("mode") {
        ...
        Ok(TextureInputWrapMode::Repeat)
    }
}
